...
const COLLAPSIBLE_ICON_CLASS = 'group-data-[collapsible=icon]';
        ...
        <div
          className={cn(
            ...
            variant === 'floating' || variant === 'inset'
              ? `${COLLAPSIBLE_ICON_CLASS}:w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4))]`
              : `${COLLAPSIBLE_ICON_CLASS}:w-[var(--sidebar-width-icon)]`,
          )}
        />
        <div
          className={cn(
            ...
            variant === 'floating' || variant === 'inset'
              ? `p-2 ${COLLAPSIBLE_ICON_CLASS}:w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)]`
              : `${COLLAPSIBLE_ICON_CLASS}:w-[var(--sidebar-width-icon)] group-data-[side=left]:border-r group-data-[side=right]:border-l`,
            className,
          )}
          {...props}
        >
  ...
  ({ className, ...props }, ref) => {
    return (
      <div
        ...
        className={cn(
          `flex min-h-0 flex-1 flex-col gap-2 overflow-auto ${COLLAPSIBLE_ICON_CLASS}:overflow-hidden`,
          className,
        )}
        {...props}
      />
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Use fixed width for deterministic rendering (complies with React purity rules)
  const width = '70%';
