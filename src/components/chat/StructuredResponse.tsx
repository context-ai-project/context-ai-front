...
export function StructuredResponse({ data, className }: StructuredResponseProps) {
  ...
  return (
    <div className={cn('space-y-4', className)} data-testid="structured-response">
      {/* Brief summary */}
      <p className="text-sm leading-relaxed">{sanitizeStructuredText(summary)}</p>
          ...
          {sections.map((section, index) => {
            ...
            return (
              <div
                ...
                data-testid={`structured-section-${section.type}`}
              >
      ...
      {relatedTopics && relatedTopics.length > 0 && (
        <div>
          ...
          <p className="text-sm text-gray-600">{relatedTopics.join(' • ')}</p>
        </div>
      )}
    </div>
  );
